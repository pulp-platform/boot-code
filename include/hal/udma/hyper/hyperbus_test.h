...
#define UDMA_HYPERBUS_OFFSET 0x1a102000+128*11
#define HYPERBUS_DEVICE_NUM 10
#define CONFIG_REG_OFFSET 0x80

static inline void set_memsel_hyper(unsigned int mem_sel){
  pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x60, mem_sel); // hyperram 0, hyperflash 1, psram(x8) 2, psram(x16)3
}

static inline int check_memsel_hyper(){
  return pulp_read32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x60);
}

static inline void set_en_latency_add(unsigned int en){
  pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x48, en ); // REG_T_EN_LATENCY_ADD enable 1, deactivated 0
}

static inline void set_t_latency_access(unsigned int latency){
  pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x44, latency); 
}

static inline void set_t_cs_max(unsigned int period){
  pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x4C, period );
}
...
static inline void set_pagebound_hyper(unsigned int page_bound){
  ...
  switch(page_bound){
     case 128:
        pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x40, 0x00 ); // page boundary is set to every 128 bytes
        break;
     case 256:
        pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x40, 0x01 ); // page boundary is set to every 256 bytes
        break;
     case 512:
        pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x40, 0x02 ); // page boundary is set to every 128 bytes
        break;
     case 1024:
        pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x40, 0x03 ); // page boundary is set to every 256 bytes
        break;
     default:
        pulp_write32(UDMA_HYPERBUS_OFFSET + CONFIG_REG_OFFSET*0 + 0x40, 0x04 ); // page boundary is not considered
  }
...
static inline int udma_hyper_id_alloc(){
  return pulp_read32(UDMA_HYPERBUS_OFFSET + 0*CONFIG_REG_OFFSET + 0x64);
}
